...
export default async function ClientDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: profile }, { data: subscriptions }, { data: appointments }, { data: plans }] = await Promise.all([
  ...
  const totalSpent = (await supabase.from('payments').select('amount').eq('client_id', id).eq('status', 'paid')).data?.reduce((s, p) => s + p.amount, 0) ?? 0
